...
impl StdioMcpClient {
    ...
    async fn spawn(&self) -> Result<()> {
        ...
        let mut child = cmd.spawn().map_err(|e| ServitorError::Mcp {
            reason: format!("failed to spawn MCP server '{}': {}", self.name, e),
        })?;

        let stdin = child.stdin.take().ok_or_else(|| ServitorError::Mcp {
            reason: "failed to capture stdin".into(),
        })?;

        let stdout = child.stdout.take().ok_or_else(|| ServitorError::Mcp {
    ...
    async fn rpc<T: serde::de::DeserializeOwned>(
        ...
        // Write to stdin
        let stdin = inner.stdin.as_mut().ok_or_else(|| ServitorError::Mcp {
        ...
        let stdout = inner.stdout.as_mut().ok_or_else(|| ServitorError::Mcp {
            reason: "MCP server not started".into(),
        })?;
        ...
        let result = response.result.ok_or_else(|| ServitorError::Mcp {
            reason: "MCP response missing result".into(),
        })?;

        serde_json::from_value(result).map_err(|e| ServitorError::Mcp {
